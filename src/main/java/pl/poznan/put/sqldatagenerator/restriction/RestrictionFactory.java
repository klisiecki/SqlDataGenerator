...
public class RestrictionFactory {
    public static Restriction createRestriction(Expression expression) {
        if (isColumnAndValueExpression(expression)) {
            if (isStringExpression(expression)) {
                return createStringRestriction(expression);
            } else {
                return createRangeRestriction(expression);
            }
            ...
            return createTwoAttributesRelationRestriction(expression);
        }
        throw new NotImplementedException("Instruction " + expression.toString() + " not implemented");
    }

    private static Restriction createRangeRestriction(Expression expression) {
    ...
    private static Restriction createStringRestriction(Expression expression) {
    ...
    private static Restriction createTwoAttributesRelationRestriction(Expression expression) {
        if (expression instanceof GreaterThan) {
